...
next_file: 
                        ...
                        if (x < 244 && finfo[x].name[0] != 0x00) {
                            // 找到这个文件了
                            p_disk = (char *) (finfo[x].clustno * 512 + 0x003e00 + ADR_DISKIMG);    // p_disk指针指向这个文件在 磁盘缓冲区 中的地址
                            cursor_x = 8;
                            // 逐字输出
                            // s = sprintf(s, "file")
                            cursor_y = cons_putstring(sheet, cursor_x, cursor_y, COL8_000000, COL8_FFFFFF, p_disk, 0, finfo[x].size);   // 文件大小，多少个字节，即多少个字符
                        } else {
                            // 没有找到文件
