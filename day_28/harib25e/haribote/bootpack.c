...
void HariMain(void) {
    ...
    *((int *) 0x0fec) = (int) &fifo;
    ...
    int i = 0,                  // 存放从缓冲区中取出数据的变量
        ...
        mmx2 = 0;
    struct SHEET *sht = 0,      // 接收遍历得到的图层的指针
            *key_win, 
            *sht2;              // sht2是应用程序运行时关闭console的专用指针
    char s[128];                // 字符串用指针
    ...
    *((int *) 0x0fe4) = (int) shtctl;
    ...
    // sht_cons
    // 改为可以创建多个命令行窗口
    int cons_size_x = 60 * 8;   // 一行显示字符数 * 8
    int cons_size_y = 25 * 16;  // 行数 * 16
    ...
    putstring8_ascii_sht(   sht_back, 
                            ...
                            COL8_C6C6C6, COL8_008484, "Harib25e");
    // 在左下角打印Terminal
    putstring8_ascii_sht(   sht_back, 
                            4, 
                            sht_back->bysize - 16 - 4, 
                            COL8_000000, COL8_C6C6C6, "Terminal");
    ...
    int vert_flag = 0,          // 纵向以及横向上是否需要改变生成新窗口方向的flag 
        hori_flag = 0, 
        button_down = 0;        // 是否按下左下角"Terminal"按钮
    // 操作系统的主循环
    while (1) { 
        // 如果存在向键盘控制器发送的数据，就发送
                ...
                */
                ...
                if (i == 0x2e && key_ctrl != 0 && key_win != 0) {    
                    ...
                    // tss.ss0 = 0时，没有应用程序在运行，此时强制结束就会出问题，需要做判断
                        ...
                        io_sti();
                        task_run(task, -1, 0);  // 如果ncst启动的任务处于休眠状态，就永远不会执行结束任务的处理
                    } else {
                    // 只是想清空命令行
                        for (j = 0; i < 256; i++) {
                ...
                // 打开新命令行窗口的指令 ctrl+n
                if (i == 0x31 && key_ctrl != 0 && cons_count != MAX_CONS_COUNT) {   // 最多16个命令行窗口
                    sht_cons = open_console(shtctl, memtotal, cons_size_x, cons_size_y);
                    // 根据上一个窗口的坐标，移动新的窗口
                    if (key_win == 0) {
                        key_win = sht_back;
                ...
                if (mouse_decode(&mdec, i) == 1) {  // mouse_decode函数返回1，即phase3已经处理完，可以显示信息
                    ...
                    if ((mdec.btn & 0x01) != 0) {
                        if (mmx < 0) {
                        // 如果处于通常模式
                            // 点到左下角"Terminal"按钮，就打开新的命令行窗口
                            // (2, sht->ysize - 24) ~ (70, sht->ysize - 4)
                            sht = shtctl->sheets[0];
                            x = mx - sht->vx0;
                            y = my - sht->vy0;
                            if (4 <= x && x <= 70 && 
                                sht->bysize - 24 <= y && y <= sht->bysize - 4) {
                                // 变成白色
                                putstring8_ascii_sht(   sht_back, 
                                                        4, 
                                                        sht_back->bysize - 16 - 4, 
                                                        COL8_FFFFFF, COL8_C6C6C6, "Terminal");
                                // 记录点击了这个按钮
                                button_down = 1;
                            }
                    ...
                    // 没有按下鼠标左键（松开鼠标左键）
                        ...
                        putstring8_ascii_sht(   sht_back, 
                                                ...
                                                sht_back->bysize - 16 - 4, 
                                                COL8_000000, COL8_C6C6C6, "Terminal");
                        if (button_down && cons_count != MAX_CONS_COUNT) {
                            button_down = 0;
                            sht_cons = open_console(shtctl, memtotal, cons_size_x, cons_size_y);
                            // 根据上一个窗口的坐标，移动新的窗口
                            if (key_win == 0) {
