...
void console_task(struct SHEET *sheet, struct SHEET *sht_back);
...
void HariMain(void) {
    ...
    *((int *) (task_cons->tss.esp + 8)) = (int) sht_back;  
    ...
    while (1) { 
        ...
        sprintf(s, "current task level: %d, priority: %d", 
                    current_task->level, 
                    current_task->priority);
        putstring8_ascii_sht(sht_back, 0, 48, COL8_FFFFFF, COL8_008484, s);
            ...
            if (i >= 256 && i < 512) {
                ...
                if (i < 0x54 && keytable[i] != 0) {
                    if (key_to == 0) {  // 发送给任务A
                        ...
                        fifo32_put(&task_cons->fifo, keytable[i] + 256);    // 还是加上256，免得和task_cons的缓冲区中其他数据冲突
                    }
                }
                if (i == 0x0e) {
                    // 按下退格键
