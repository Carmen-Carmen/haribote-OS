...
void HariMain(void) {
    ...
    *((int *) (task_cons->tss.esp + 4)) = (int) sht_cons;  // 将sht_back作为task_b_main的参数，手动压栈
...
void console_task(struct SHEET *sheet) {
    struct FIFO32 fifo;
    struct TIMER *timer;
    struct TASK *task = task_now(); // 后续要进行唤醒和休眠task_cons的操作，所以要获取task_cons；但是作为参数传入又太繁琐；因此如果在执行这个函数，就说明当前任务就是task_cons，所以直接用task_now()来获取就行

    int i, fifobuf[128], cursor_x = 8, cursor_c = COL8_000000;
    fifo32_init(&fifo, 128, fifobuf, task); // 有数据写入时，唤醒task_cons
    timer = timer_alloc();
    timer_init(timer, &fifo, 1);
    timer_settime(timer, 50);
