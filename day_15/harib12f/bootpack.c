...
void HariMain(void) {
    ...
    task_b_esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024 - 8;    // 栈底的内存地址（栈底是高地址；栈顶是低地址！）
    *((int *) (task_b_esp + 4)) = (int) sht_back;   // 将sht_back地址压入task_b的栈空间中[ESP + 4]地址，当task_b_main被调用时，sht_back可作为这个函数的参数被传入
...
void task_b_main(void) {
    struct FIFO32 fifo;
    struct TIMER *timer_ts, *timer_put, *timer_1s; // ts是任务切换定时；put是输出定时
