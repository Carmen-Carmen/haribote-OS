            ...
            if (i <= 1) {
                    ...
                    if (multi_line == 0) {      
                        ...
                        if (cursor_x > 8) {     // 在当前行内退格
                            ...
                            */
                            ...
                            int s_index;
                            if (multi_line != 0) {
                                for (s_index = (240 - 16) / 8 + (multi_line - 1) * 240 / 8; s_index <= cmdindex; s_index++) {
                                    s[s_index - ((240 - 16) / 8 + (multi_line - 1) * 240 / 8)] = cmdline[s_index];
                                }
                                s[240 / 8] = 0;
                ...
                // 回车键
                    ...
                    if (strcmp(cmdline, "mem") == 0) {
                        ...
                        // "clear" command
                        cons_clear(sheet, 8, 28, 240, 128);
                        cursor_y = 28; 
                    } else if (cmdline[0] != 0) {
                        // 既不是命令，也不是空行
                        putstring8_ascii_sht(sheet, 8, cursor_y, COL8_FFFFFF, COL8_000000, "command not found!");
                        cursor_y = cons_newline(cursor_y, sheet);
                        cursor_y = cons_newline(cursor_y, sheet);
                    }
                ...
                // 一般字符
                    ...
                    cmdline[cmdindex++] = i;  // 一个字符宽8，减去文本框到窗口边距、命令行提示符以及1个空格
