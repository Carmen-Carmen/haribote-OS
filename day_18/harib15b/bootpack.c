...
void console_task(struct SHEET *sheet) {
    ...
    int i, fifobuf[128], cursor_x = 24, cursor_c = -1;  // 将cursor_c初始化成-1，由后面主循环来判断是否要开始光标闪烁
    char *s = NULL;
    ...
    for (;;) {
        ...
        if (fifo32_status(&task->fifo) == 0) {
            ...
            if (i <= 1) {
                ...
                // 停止光标闪烁
                cursor_c = -1;
                boxfill8(sheet->buf, sheet->bxsize, COL8_000000, cursor_x, 28, cursor_x + 7, 43);   // 用黑色将光标隐去
            }
            // 重新显示光标
            if (cursor_c >= 0) {
