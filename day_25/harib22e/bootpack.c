...
void HariMain(void) {
    ...
    *((int *) 0x0fe4) = (int) shtctl;
    ...
    for (i = 0; i < 2; i++) {   // 先创建2个
        ...
        buf_cons[i] = (unsigned char *) memman_alloc_4k(memman, (8 + cons_size_x + 8) * (28 + cons_size_y + 9));
        sheet_setbuf(sht_cons[i], buf_cons[i], 8 + cons_size_x + 8, 28 + cons_size_y + 9, -1);
        sprintf(s, "task_b%d", i);
        make_window8(buf_cons[i], 8 + cons_size_x + 8, 28 + cons_size_y + 9, "console", 0);
        make_textbox8(sht_cons[i], 8, 28, cons_size_x, cons_size_y, COL8_000000);
        task_cons[i] = task_alloc();
        task_cons[i]->tss.esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024 - 20;   // 分配栈内存，压了新的参数进去记得要改栈顶啊阿啊！
        task_cons[i]->tss.eip = (int) &console_task;   // 指令指针指向console_task
        ...
        *((int *) (task_cons[i]->tss.esp + 8)) = memtotal;         // 将memtotal作为第二个参数传入cons任务的入口函数
        *((int *) (task_cons[i]->tss.esp + 12)) = cons_size_x;
        *((int *) (task_cons[i]->tss.esp + 16)) = cons_size_y;
    ...
    sheet_slide(sht_cons[0], 100, 200);
    sheet_slide(sht_cons[1], 300, 200);
    sheet_updown(sht_back,  0);  // 显示背景
    sheet_updown(sht_cons[0],  1);
    sheet_updown(sht_cons[1],  2);
    sheet_updown(sht_win,   3); 
    sheet_updown(sht_mouse, 4);  // 显示鼠标
    // 显示鼠标坐标（左上角）
            ...
            if (i >= 256 && i < 512) {
                ...
                */
                ...
                if (i == 0x2e && key_ctrl != 0) {    
                    if (task_cons[0]->tss.ss0 != 0) {
                    // tss.ss0 = 0时，没有应用程序在运行，此时强制结束就会出问题，需要做判断
                        cons = (struct CONSOLE *) *((int *) 0x0fec);
                        cons_putstring(cons, "\nBreak (key) :\n", COL8_FFFFFF, COL8_840000, -1);
                        io_cli();   // 要改变寄存器值了，此时不能切换到其他任务，先关中断
                        task_cons[0]->tss.eax = (int) &(task_cons[0]->tss.esp0);  // 设置eax为esp0的地址
                        task_cons[0]->tss.eip = (int) asm_end_app;             // 这样asm_end_app中就能得到esp0的值
                        io_sti();
                    } else {
                    // 只是想清空命令行
                        for (j = 0; i < 256; i++) {
                            fifo32_put(&task_cons[0]->fifo, 256 + 8);
                        }
                    }
                }
