...
void HariMain(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	buf_cons = (unsigned char *) memman_alloc_4k(memman, 816 * 677);
	sheet_setbuf(sht_cons, buf_cons, 816, 677, -1); /* 透明色なし */
	make_window8(buf_cons, 816, 677, "console", 0);
	make_textbox8(sht_cons, 8, 28, 800, 640, COL8_000000);
				...
				if (i == 256 + 0x0e) {	/* バックスペース */
					if (key_to == 0) {	/* タスクAへ */
						if (cursor_x > 8) {
							...
							putstring8_ascii_sht(sht_win, cursor_x, 28, COL8_000000, COL8_FFFFFF, " ");
							cursor_x -= 8;
						}
					} else {	/* コンソールへ */
