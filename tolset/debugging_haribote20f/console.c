...
void console_task(struct SHEET *sheet, unsigned int memtotal)
{
	...
	*((int *) 0x0fec) = (int) &cons;
	...
	file_decodefat(fat, (unsigned char *) (ADR_DISKIMG + 0x000200));

	/* プロンプト表示 */
	cons_putchar(&cons, '>', 1, COL8_FFFFFF, COL8_000000);
			...
			if (256 <= i && i <= 511) { /* キーボードデータ（タスクA経由） */
				if (i == 8 + 256) {
					...
					if (cons.cur_x > 16) {
						...
						cons_putchar(&cons, ' ', 0, COL8_FFFFFF, COL8_000000);
						cons.cur_x -= 8;
					}
				} else if (i == 10 + 256) {
					/* Enter */
					...
					if (cons.cur_x < 800) {
						...
						cons_putchar(&cons, i - 256, 1, COL8_FFFFFF, COL8_000000);
					}
				}
			}
			/* カーソル再表示 */
...
void cons_newline(struct CONSOLE *cons)
{
	...
	if (cons->cur_y < 28 + 624) {
		...
		for (y = 28 + 624; y < 28 + 640; y++) {
			for (x = 8; x < 8 + 800; x++) {
				sheet->buf[x + y * sheet->bxsize] = COL8_000000;
			}
		}
		sheet_refresh(sheet, 8, 28, 8 + 800, 28 + 640);
	}
	cons->cur_x = 8;
	return;
}
...
void cmd_top(struct CONSOLE *cons) {
    ...
    for (i = 0; i < MAX_TASKLEVELS; i++) {
        for (j = 0; j < MAX_TASKS_LV; j++) {
            ...
            if (task->flags == 1 || task->flags == 2) {
                sprintf(s, "level: %d, No. %d, flags: %d\n", i, j, task->flags);
                cons_putstr0(cons, s);
            }
        }
    }
