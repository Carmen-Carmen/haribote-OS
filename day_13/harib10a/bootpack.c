...
void HariMain(void) {
    ...
    fifo8_init(&timerfifo1, 8, timerbuf1);   // 初始化timer的缓冲区
    timer1 = timer_alloc();                 // 分配timer
    timer_init(timer1, &timerfifo1, 1);     // 初始化timer
    ...
    while (1) { // 有人喜欢写for(;;)，说是因为编译出来的运行快；但实际上编译器会把while (1)编译成一样快的语句来执行
        ...
        */
        putstring8_ascii_sht(sht_win, 40, 60, COL8_000000, COL8_C6C6C6, s);

        // 显示一下timerctl.using
        sprintf(s, "using: %03d", timerctl.using);
        /*
        boxfill8(buf_back, binfo->scrnx, COL8_008484, 0, 112, 71, 127);
        putstring8_ascii(buf_back, binfo->scrnx, 0, 112, COL8_FFFFFF, s);
        sheet_refresh(sht_back, 0, 112, 72, 128);
        */
        putstring8_ascii_sht(sht_back, 0, 112, COL8_FFFFFF, COL8_008484, s);


        // 先关中断，再作处理，不然就乱了
        io_cli();
        
        if (    // 如果所有缓冲区中都没有数据，就开中断
            fifo8_status(&keyfifo) + 
            fifo8_status(&mousefifo) + 
            fifo8_status(&timerfifo1) + 
            ...
            if (fifo8_status(&keyfifo) != 0) {
                ...
                */
                ...
                io_sti();
                putstring8_ascii(buf_back, binfo->scrnx, 0, 64, COL8_FFFFFF, "10[sec]");
                sheet_refresh(sht_back, 0, 64, 56, 80);
            } else if (fifo8_status(&timerfifo2) != 0) {
                i = fifo8_get(&timerfifo2);
                io_sti();
                putstring8_ascii(buf_back, binfo->scrnx, 0, 80, COL8_FFFFFF, "3[sec]");
