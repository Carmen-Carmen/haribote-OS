    ...
    for (;;) {
        ...
        if (fifo32_status(&task->fifo) == 0) {
            ...
            if (i <= 1) {
                ...
                // 上方向键
                    // sprintf(s, "currentcmd: %d, cmd_updown: %d, target: %d\n", currentcmd, cmd_updown, currentcmd + cmd_updown);
                    // cons_putstring(&cons, s, -1);
...
void cmd_top(struct CONSOLE *cons) {
    ...
    //         task = taskctl->level[i].tasks[j];
    //         if (task->flags != 0) {
    //             sprintf(s, "level: %d, No. %d, flags: %d, sel: %d\n", i, j, task->flags, task->sel);
    //             cons_putstring(cons, s, -1);
    //         }
    //     }
    // }
