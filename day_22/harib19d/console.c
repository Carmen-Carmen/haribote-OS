...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline) {
    struct MEMMAN *memman = (struct MEMMAN *) MEMMAN_ADDR;
    char *p_disk, *q_disk;  // p, q是指向磁盘缓冲区的俩指针
    char name[18];
    ...
    if (finfo != 0) {
        p_disk = (char *) memman_alloc_4k(memman, finfo->size);
        q_disk = (char *) memman_alloc_4k(memman, 64 * 1024);   
        *((int *) 0xfe8) = (int) p_disk;    // 将p_disk指向的可执行文件地址存放在内存地址0xfe8中，方便向它传递代码段等信息
        file_loadfile(finfo->clustno, finfo->size, p_disk, fat, (char *) (ADR_DISKIMG + 0x003e00));
        set_segmdesc(gdt + 1003, finfo->size - 1,   (int) p_disk, AR_CODE32_ER + 0x60); // 将访问权限加上0x60就将该段设置为应用程序用
        set_segmdesc(gdt + 1004, 64 * 1024 - 1,     (int) q_disk, AR_DATA32_RW + 0x60);

        // 如果4～7字节的内容为"Hari"，则一定是通过bim2hrb工具转化产生的hrb文件，需要从0x1b开始执行
        if (finfo->size >= 8 && strncmp(p_disk + 4, "Hari", 4) == 0) {
            // 因为有start_app来帮忙指定开始执行的eip，就不用把开头6个字符改成"JMP 0x1b"了
            start_app(0x1b, 1003 * 8, 64 * 1024, 1004 * 8, &(task->tss.esp0));
        } else {
            start_app(0, 1003 * 8, 64 * 1024, 1004 * 8, &(task->tss.esp0)); // 用于启动应用程序的函数，本质是在执行far-CALL之前设置ESP和DS/SS
        }

        memman_free_4k(memman, (int) p_disk, finfo->size);
        memman_free_4k(memman, (int) q_disk, 64 * 1024);
        cons_newline(cons);
        return 1;
    } else {
        // 没找到
...
int * hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax) {
    ...
    } else if (edx == 4) {  // 应用程序结束的api，edx = 4时结束
        return &(task->tss.esp0);   // 应用程序结束，返回OS栈地址esp0
    } 

/*
    else if (edx == 123456789) {
        *((char *) 0x00102600) = 0; // 相当于自己在操作系统里留了个漏洞，理论上源码是不会被修改的
    }
