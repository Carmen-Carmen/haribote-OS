...
struct BOOTINFO {
    ...
    char *vram;                         // 1个dword，char *是指针变量，在32位处理器上占4个字节，因为寻址空间是2^32，即4个BYTE
};
...
void HariMain(void) {
    ...
    // 32位模式下不能依靠BIOS显示字符
    // 8 * 16的长方形像素点阵来表示字母A，即16个2位的16进制数（8位、2个BYTE）的数组
    ...
    // 画个字
    putfont8(binfo->vram, binfo->scrnx, 10, 10, COL8_848484, font_A);
