...
void HariMain(void) {
    ...
    // 也可以直接输出字符串了！
    putstring8_ascii(binfo->vram, binfo->scrnx, 8, 8, COL8_FFFFFF, "ABC 123");
    putstring8_ascii(binfo->vram, binfo->scrnx, 31, 31, COL8_000000, "Haribote OS.");
    putstring8_ascii(binfo->vram, binfo->scrnx, 30, 30, COL8_FFFFFF, "Haribote OS.");

    // 尝试输出变量的值
    char *s;
    sprintf(s, "resolution: %d * %d", binfo->scrnx, binfo->scrny);  // 字符串地址，格式，填充占位符的值
    putstring8_ascii(binfo->vram, binfo->scrnx, 16, 64, COL8_FFFFFF, s);
    sprintf(s, ", vram address: %8x", binfo->vram);
    putstring8_ascii(binfo->vram, binfo->scrnx, 16, 80, COL8_FFFFFF, s);
