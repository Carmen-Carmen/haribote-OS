; haribote-ipl
; 这个initial program loader启动程序加载器最多只能读10个扇区！所以文件名是ipl10
; TAB=4
        
CYLS    EQU     10                  ; 一共读到哪个柱面，相当于声明一个常量 cyls equals 10
...
retry:
        ...
        JNC     next                 ; Jump if Not Carry，如果没有错误，就跳转到next部分
        ; 到这边就是有错误了
...
next:   
        ; 往后读扇区
        ; 1. 将缓冲区地址ES:BX向后移动512Byte（0x200）
        MOV     AX, ES              
        ADD     AX, 0x0020
        MOV     ES, AX              ; 因为ES寄存器不能接受ADD指令，所以要先把ES寄存器中数据放到AX中，加完再放回来
        ; 2. 扇区向后移动1个
        ADD     CL, 1
        CMP     CL, 18              ; 注意一共只有18个扇区
        JBE     readloop            ; Jump if Below or Equal，跳转回readloop循环的开头；否则读取完毕
        
        ; 换一个磁头读，即读当前柱面的反面
        MOV     CL, 1               ; 重新把CL即扇区置为1
        ADD     DH, 1               ; 磁头号加1
        CMP     DH, 2               ; 一共只有2个磁头
        JB      readloop            ; Jump if Below，如果磁头号<2就跳回readloop，注意此时CL也就是扇区号为1，因此会重复“往后读扇区”部分的代码，直到读完第18个扇区

        ; 向内读柱面
        MOV     DH, 0               ; 重新把磁头号置为0
        ADD     CH, 1               ; CH加1，即柱面号加1
        CMP     CH, CYLS            ; CYLS=10
        JB      readloop            ; 如果还没读到第10柱面，就跳转回readloop，此时会从该柱面的0号磁头（正面）的第一扇区开始读       
       
; 读盘完成后执行haribote.sys
        MOV     [0x0ff0], CH        ; 记录IPL（启动程序加载器）究竟读取到哪个柱面，记在地址号0x0ff0的内存中
        JMP     0xc200
