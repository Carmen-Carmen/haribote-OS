...
void HariMain(void) {
    ...
    unsigned char *buf_back, buf_mouse[MOUSE_XSIZE * MOUSE_YSIZE];
    ...
    sheet_refresh(shtctl, sht_back, 0, 0, binfo->scrnx, 48);  // 局部刷新back图层中(0, 0) ~ (320, 48)，即3行字的区域
    ...
    while (1) {
        ...
        if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) == 0) {
            ...
            if (fifo8_status(&keyfifo) != 0) {
                ...
                sheet_refresh(shtctl, sht_back, 0, 16, 16, 32);             // 局部刷新键盘按键信息
                ...
                if (mouse_decode(&mdec, i) == 1) {  // mouse_decode函数返回1，即phase3已经处理完，可以显示信息
                    ...
                    sheet_refresh(shtctl, sht_back, 32, 16, 32 + 8 * 13, 32);   // 局部刷新鼠标信息
                    ...
                    sheet_refresh(shtctl, sht_back, 0, 0, 80, 16);  // 局部刷新鼠标坐标
