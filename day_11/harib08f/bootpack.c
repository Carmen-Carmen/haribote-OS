...
void HariMain(void) {
    ...
    sheet_setbuf(sht_win, buf_win, 160, 68, -1);                        // 窗口图层没有透明色
    init_screen8(buf_back, binfo->scrnx, binfo->scrny);
    init_mouse_cursor8(buf_mouse, 99);  // 初始化鼠标，设定透明色号99
    make_window8(buf_win, 160, 68, "window");   // 在窗口图层缓冲区中打印窗口
    ...
    sheet_updown(sht_win, 2);   // 显示窗口
    sheet_updown(sht_mouse, 1); // 显示鼠标
    // 显示鼠标坐标（左上角）
    sprintf(s, "(%d, %d)", mx, my);
...
void make_window8(unsigned char *buf, int xsize, int ysize, char *title) {
    ...
    // 打印窗口标题，打印在窗口的中间吧
    putstring8_ascii(buf, xsize, 24, 4, COL8_FFFFFF, title);   // 使用string.h中strlen得到title长度
    // 在窗口左上角打印关闭按钮
    for (y = 0; y < 14; y++) {
        for (x = 0; x < 16;x++) {
            c = closebtn[y][x]; 
