...
void init_gdtidt(void) {
	...
	set_segmdesc(gdt + 1, 0xffffffff, 0x00000000, AR_DATA32_RW);  // 将1段上限值设为0xffffffff（4GB），基址设为0，访问权限设为0x4092（有意义，还未知）
	...
	load_gdtr(LIMIT_GDT, ADR_GDT);  // naskfunc.asm中编写的将GDT信息写入GDTR的方法，包括了内存的开始地址（0x00270000）和个数（0xffff，8192个）
