...
void HariMain(void) {
    ...
    *((int *) 0x0fe4) = (int) shtctl;
    ...
    int *cons_fifo[2];
    int cons_size_x = 50 * 8;   // 一行显示字符数 * 8
...
task_cons[i]->tss.es = 1 * 8;
    ...
    sheet_slide(sht_cons[0], 50, 50);
    sheet_slide(sht_cons[1], 558, 50);
    ...
    key_win = sht_cons[0];      // 改用key_win指向当前focus的窗口
    keywin_on(key_win);
        ...
        if (fifo32_status(&fifo) == 0) {
            ...
            if (i >= 256 && i < 512) {
                ...
                */
                ...
                if (s[0] != 0 && key_ctrl == 0) {    // 一般字符&回车键&退格键
...
void keywin_on(struct SHEET *key_win) {
    ...
    if ((key_win->flags & 0x20) != 0) {
        fifo32_put(&key_win->task->fifo, 2);    // 命令行窗口光标off
    }
}
