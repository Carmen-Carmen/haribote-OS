...
void console_task(struct SHEET *sheet, unsigned int memtotal, int size_x, int size_y) {
    ...
    char **cmdlines = (char **)memman_alloc_4k(memman, 256 * 2048);  // 保存enter过的命令
...
// 执行命令
    if (strcmp(cmdline, "mem") == 0) {
        ...
        cmd_top(cons);
        cons_newline(cons);
    } else if (strcmp(cmdline, "printAB") == 0) {
        // "printAB" command
        cmd_printAB();
        cons_newline(cons);
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline) {
    ...
    if (finfo != 0) {
        ...
        if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
            ...
            set_segmdesc(gdt + task->sel / 8 + 1000, finfo->size - 1,   (int) p, AR_CODE32_ER + 0x60); // 将访问权限加上0x60就将该段设置为应用程序用
            ...
            start_app(  0x1b,                   // eip指向0x1b，从这条指令开始执行
                        task->sel + 1000 * 8,   // 代码段cs, 结果落在(1003, 2002)
                        esp,                    // esp初始值
                        task->sel + 2000 * 8,   // 数据段ds, 结果落在(2003, 3002)
                        &(task->tss.esp0));
...
void cmd_top(struct CONSOLE *cons) {
    ...
    for (i = 0; i < MAX_TASKS; i++) {
        ...
        if (task->flags != 0) {
            sprintf(s, "task[%d], level[%02d], priority[%2d], flags[%d], sel[%d]\n", i, task->level, task->priority, task->flags, task->sel);
            cons_putstring(cons, s, -1);
        }
    }
...
void cmd_printAB(void) {
    struct MEMMAN *memman = (struct MEMMAN *) MEMMAN_ADDR;
    struct TASK *task_demo_A, *task_demo_B;
    task_demo_A = task_alloc();
    task_demo_B = task_alloc();
    task_demo_A->tss.esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024;
    task_demo_B->tss.esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024;
    task_demo_A->tss.eip = (int) &demo_A;
    task_demo_B->tss.eip = (int) &demo_B;
    task_demo_A->tss.es = 1 * 8;
    task_demo_A->tss.cs = 2 * 8;
    task_demo_A->tss.ss = 1 * 8;
    task_demo_A->tss.ds = 1 * 8;
    task_demo_A->tss.fs = 1 * 8;
    task_demo_A->tss.gs = 1 * 8;
    task_demo_B->tss.es = 1 * 8;
    task_demo_B->tss.cs = 2 * 8;
    task_demo_B->tss.ss = 1 * 8;
    task_demo_B->tss.ds = 1 * 8;
    task_demo_B->tss.fs = 1 * 8;
    task_demo_B->tss.gs = 1 * 8;
    task_run(task_demo_A, 3, 2);
    task_run(task_demo_B, 3, 2);
...
void cmd_history(struct CONSOLE *cons, char **cmdlines, int currentcmd) {
    ...
    for (i = 0; i < currentcmd; i++) {
        cons_putstring(cons, cmdlines[i], -1);
        cons_newline(cons);
    }
    return;
}
    ...
    */
    ...
    if (edx == 1) {
        ...
        */
        ...
        sheet_slide(sht, (cons_sht->bxsize - esi) / 2 + cons_sht->vx0, (cons_sht->bysize - edi) / 2 + cons_sht->vy0); // 移动到命令行窗口的中央
