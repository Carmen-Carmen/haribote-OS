...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1) {
    int h, bx, by, vx, vy, bx0, by0, bx1, by1, 
        bx2, sid4, i, i1, *p, *q, *r;           // 用于加速refresh_sub函数，原理同refresh_map; p, q, r分别是map、vram和sht->buf的指针
    unsigned char   *buf, 
                    *vram = ctl->vram, 
                    *map = ctl->map, 
                    sid;
    ...
    for (h = h0; h <= h1; h++) {   
        ...
        if ((sht->vx0 & 3) == 0) {
            // sht->vx0能被4整除，可以通过写入4 Byte的数据来加速
            i = (bx0 + 3)   / 4;    // bx0除4后小数进位
            i1 = bx1        / 4;    // bx0除4后小数舍去; bx0 ~ i以及i1 ~ bx1部分会分别逐个Byte渲染
            i1 = i1 - i;
            sid4 = sid | sid << 8 | sid << 16 | sid << 24;  // 4个Byte的sid
            ...
            for (by = by0; by < by1; by++) {
                ...
                for (i = 0; i < i1; i++) {
                    // 绘制4的倍数部分
                    if (p[i] == sid4) {
                        q[i] = r[i];    // 大多数（被4整除？）是这种情况，所以速度会变快
                    } else {
                        bx2 = bx + i * 4;
                        vx = sht->vx0 + bx2;
        ...
        if (sht->col_inv == -1) {
            ...
            if ((sht->vx0 & 3) == 0 && (bx0 & 3) == 0 && (bx1 & 3) == 0) {
                ...
                for (by = by0; by < by1; by++) {
                    vy = sht->vy0 + by; // 确定当轮循环渲染的y坐标（画面中）
                    vx = sht->vx0 + bx0;
                    p = (int *) &map[vy * ctl->xsize + vx]; // 指向当轮渲染的坐标
                    for (bx = 0; bx < bx1; bx++) {
                        p[bx] = sid4;   // 直接MOV 4 Byte的数据进去
                    }
                }
            } else {
                for (by = by0; by < by1; by++) {
