...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal, int cons_size_x, int cons_size_y);
...
void HariMain(void) {
    ...
    *((int *) 0x0fe4) = (int) shtctl;
    ...
    sht_back->task = task_a;

    // sht_cons
    // 改为可以创建多个命令行窗口
    int cons_size_x = 50 * 8;   // 一行显示字符数 * 8
    int cons_size_y = 20 * 16;  // 行数 * 16
    struct SHEET *sht_cons[16];
    sht_cons[0] = open_console(shtctl, memtotal, cons_size_x, cons_size_y);
    int cons_index;
    for (cons_index = 1; cons_index < 64; cons_index++) {
        sht_cons[cons_index] = 0;
    }
    cons_index = 0;
        ...
        if (fifo32_status(&fifo) == 0) {
            ...
            if (i >= 256 && i < 512) {
                ...
                */
                ...
                // 打开新命令行窗口的指令 ctrl+n
                if (i == 0x31 && key_ctrl != 0 && cons_index != 16) {
                    sht_cons[++cons_index] = open_console(shtctl, memtotal, cons_size_x, cons_size_y);
                    // 根据上一个窗口的坐标，移动新的窗口
                    x = key_win->vx0;
                    y = key_win->vy0;
                    ...
                    sheet_slide(sht_cons[cons_index], x, y);
                    sheet_updown(sht_cons[cons_index], shtctl->top);
                }
            } else if (i >= 512 && i < 768) {
                // 512 <= i < 768: 鼠标中断
                i -= 512;
...
struct SHEET *open_console( 
    ...
    task->tss.esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024 - 20;   // 分配栈内存，压了新的参数进去记得要改栈顶啊阿啊！
